...
inline BosonStar::BosonStar(BosonStar_params_t a_params_BosonStar,
    ...
    : m_dx(a_dx), m_G_Newton(a_G_Newton),
      ...
      m_params_potential(a_params_potential), m_identical(a_identical),
...
template <class data_t> void BosonStar::compute(Cell<data_t> current_cell) const
{
    ...
    if (binary)
    {
        ...
        // Again, finding the values to be substracted from position of star 1,
        // that is below we find the effect of star 2 on star 1
        ...
        double pc_os_p2 =
            psi_p2 * psi_p2 * c_ * c_ - omega_p2 * omega_p2 * s_ * s_;
    ...
    if (initial_data_choice == 0)
    {
        ...
        FOR2(i, j)
        vars.A[i][j] =
            vars.chi * (KLL[i][j] - one_third * vars.K * gammaLL[i][j]);
    ...
    if (initial_data_choice == 2)
    {
        ...
        double pc_os_22 =
            psi_22 * psi_22 * cosh(-rapidity2) * cosh(-rapidity2) -
        ...
        // This is \chi(x_A)
        double chi_1 = pow(
            superpose_1[0][0] * superpose_1[1][1] * superpose_1[2][2], n_power);
        // This is \chi(x_B)
        double chi_2 = pow(
            superpose_2[0][0] * superpose_2[1][1] * superpose_2[2][2], n_power);
        ...
        double profile_12 = weight.profile_chi(argument_xB_xA, argument_yB_yA,
                                               0., radius_width1);

        double argument_xA_xB =
            (separation / (q + 1)) * (-q * cosh(rapidity) - cosh(-rapidity2));
        double argument_yA_yB = impact_parameter;
        double profile_21 = weight.profile_chi(argument_xA_xB, argument_yA_yB,
                                               0., radius_width2);
