        ...
        data_t &dVdmodulus_phi_squared, const vars_t<data_t> &vars) const
    {
        ...
        else // else star is solitonic
        {
            ...
            V_of_modulus_phi_squared = m_params.scalar_mass * m_params.scalar_mass *
                modulus_phi_squared*
                pow(1.-2.*modulus_phi_squared/(m_params.sigma_solitonic*m_params.sigma_solitonic),2);

            dVdmodulus_phi_squared = m_params.scalar_mass * m_params.scalar_mass *
                (1.-6.*modulus_phi_squared/(m_params.sigma_solitonic*m_params.sigma_solitonic))*
                (1.-2.*modulus_phi_squared/(m_params.sigma_solitonic*m_params.sigma_solitonic));
        }
    }
};
