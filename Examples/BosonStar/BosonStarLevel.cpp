...
void BosonStarLevel::initialData()
{
    ...
    BosonStar boson_star(m_p.bosonstar_params, m_p.bosonstar2_params,
                                     ...
                                     const double a_time)
{
    ...
    //    m_p.formulation, m_p.G_Newton);
    MatterCCZ4RHS<ComplexScalarFieldWithPotential,
                  IntegratedMovingPunctureGauge, FourthOrderDerivatives>
        my_ccz4_matter(complex_scalar_field, m_p.ccz4_params, m_dx, m_p.sigma,
                       m_p.formulation, m_p.G_Newton);
...
void BosonStarLevel::specificPostTimeStep()
{
    ...
    // m_level==m_p.angmomflux_params.extraction_level)
    double temp_dx;
    // if (m_p.do_flux_integration &&
    ...
    //         getAMRLevelHierarchy().stdVector(); std::vector<double>
    //         S_phi_integrals(m_p.angmomflux_params.num_extraction_radii); //
    //         vector storing all integrals std::vector<double>
    //         Q_phi_integrals(m_p.angmomflux_params.num_extraction_radii); //
    //         vector storing all integrals
    ...
    //             m_p.angmomflux_params.center,
    //                                  c_Fphi_flux, c_Sphi_source,
    //                                  c_Qphi_density,
    //                                                  c_rho,
    //                                                  Interval(c_s1,c_s3),
    ...
    // std::vector<string>
    // title_line(m_p.angmomflux_params.num_extraction_radii); string
    // dummy_string; for (int j=0; j<m_p.angmomflux_params.num_extraction_radii;
    // j++)
    // {
    //     dummy_string = "r = " +
    //     to_string(m_p.angmomflux_params.extraction_radii[j]); title_line[j] =
    //     dummy_string;
    // }
    ...
    // std::vector<string>
    // title_line2(m_p.angmomflux_params.num_extraction_radii); string
    // dummy_string2; for (int j=0;
    // j<m_p.angmomflux_params.num_extraction_radii; j++)
    // {
    //     dummy_string2 = "r = " +
    //     to_string(m_p.angmomflux_params.extraction_radii[j]); title_line2[j]
    //     = dummy_string2;
    // }
    ...
    //         AngMomFlux
    //         ang_mom_flux(m_p.angmomflux_params,m_time,m_dt,m_restart_time,first_step);
